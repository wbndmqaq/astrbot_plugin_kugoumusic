...
# 常驻 Chromium：进程内复用同一浏览器实例，避免每次渲染都启动/关闭一个 Chromium
_playwright = None
_browser = None
_lock = asyncio.Lock()
...
async def _get_browser():
    ...
    async with _lock:
        ...
        try:
            ...
            _browser = await pw.chromium.launch(args=_LAUNCH_ARGS)
            _playwright = pw
...
async def close_browser() -> None:
    """关闭常驻浏览器与 playwright 运行时并清空全局（插件卸载/重载时调用）。"""
    global _playwright, _browser
    if _browser is not None:
        with contextlib.suppress(Exception):
...
async def _render_once(browser, html: str) -> bytes:
    """在给定浏览器上渲染一页并截图（页面用完即关，浏览器保持常驻）。"""
...
async def render_card_png(tmpl_path: str, data: dict) -> bytes | None:
    ...
    复用常驻 Chromium（见 ``_get_browser``）；浏览器若已失效则丢弃缓存重建后重试一次。
    """
    ...
    if browser is None:
        return None
    try:
        return await _render_once(browser, html)
    except Exception as e:  # noqa: BLE001
        # 常驻浏览器可能已崩溃/被外部关闭：丢弃缓存重建后重试一次
        logger.warning(f"{_LOGGER_TAG} 渲染失败，重建浏览器后重试: {e}")
        await close_browser()
    browser = await _get_browser()
    if browser is None:
        return None
